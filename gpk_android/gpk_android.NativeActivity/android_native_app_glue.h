...
#ifndef GPK_ANDROID_NATIVE_APP_GLUE_H_2907349823
#define GPK_ANDROID_NATIVE_APP_GLUE_H_2907349823
...
// The 'threaded_native_app' static library is used to provide a different execution model where the application can implement its own main event loop in a different thread instead. Here's how it works:
...
//
// 4/ Whenever you receive a LOOPER_ID_MAIN or LOOPER_ID_INPUT event, the returned data will point to an android_poll_source structure. You can call the process() function on it, and fill in android_app->onAppCmd
//    and android_app->onInputEvent to be called for your own processing of the event.
//
...
struct android_poll_source {
    ...
    struct android_app				* app;					// The android_app this ident is associated with.
    void							(*process)							(struct android_app* app, struct android_poll_source* source);	// Function to call to perform the standard processing of data from this source.
};
...
struct android_app {
	...
	void							(*onAppCmd)							(struct android_app* app, int32_t cmd);			// Fill this in with the function to process main app commands (APP_CMD_*)
	int32_t							(*onInputEvent)						(struct android_app* app, AInputEvent* event);	// Fill this in with the function to process input events. At this point the event has already been pre-dispatched, and it will be finished upon return. Return 1 if you have handled the event, 0 for any default dispatching.
...
enum 
    // Looper data ID of commands coming from the app's main thread, which is returned as an identifier from ALooper_pollOnce(). The data for this identifier is a pointer to an android_poll_source structure.
    // These can be retrieved and processed with android_app_read_cmd() and android_app_exec_cmd().
	{ LOOPER_ID_MAIN		= 1

    // Looper data ID of events coming from the AInputQueue of the application's window, which is returned as an identifier from ALooper_pollOnce().  The data for this identifier is a pointer to an android_poll_source structure. 
...
enum 
    ...
    , APP_CMD_DESTROY				// Command from main thread: the app's activity is being destroyed, and waiting for the app thread to clean up and exit before proceeding. 
		...
		int8_t						android_app_read_cmd				(struct android_app* android_app);				// Call when ALooper_pollAll() returns LOOPER_ID_MAIN, reading the next app command message. 
		void						android_app_pre_exec_cmd			(struct android_app* android_app, int8_t cmd);	// Call with the command returned by android_app_read_cmd() to do the initial pre-processing of the given command.  You can perform your own actions for the command after calling this function. 
		void						android_app_post_exec_cmd			(struct android_app* android_app, int8_t cmd);	// Call with the command returned by android_app_read_cmd() to do the final post-processing of the given command.  You must have done your own actions for the command before calling this function. 

extern	void						android_main						(struct android_app* app);	// This is the function that application code must implement, representing the main entry to the app. 

#ifdef __cplusplus
}
#endif

#endif // GPK_ANDROID_NATIVE_APP_GLUE_H_2907349823 
